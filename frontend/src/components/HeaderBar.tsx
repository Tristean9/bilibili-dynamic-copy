...
const LeftEntry: React.FC<EntryProps> = ({ entry }) => {
	if (entry.title === "首页") {
		return (
			<a href={entry.url} target="_blank" className="entry-title">
				{entry.icon ? (
					<div
	...
	} else if (entry.title === "下载客户端") {
		return (
			<a
				href={entry.url}
				target="_blank"
				className="download-entry download-client-trigger"
			>
				{entry.icon ? (
					<div
						dangerouslySetInnerHTML={{ __html: entry.icon }}
		...
		return (
			<a href={entry.url} target="_blank" className="default-entry">
				<span> {entry.title} </span>
			</a>
		);
	}
...
const HeaderBarLeftEntries: React.FC = () => {
	const leftEntry = [
		{
			id: 1,
			title: "首页",
		...
		{ id: 2, title: "番剧", url: "https://www.bilibili.com/anime/" },
		{ id: 3, title: "直播", url: "https://live.bilibili.com/" },
		{ id: 4, title: "游戏中心", url: "https://game.bilibili.com/platform" },
		{
			id: 5,
			title: "会员购",
			url: "https://show.bilibili.com/platform/home.html?msource=pc_web",
		},
		{
			id: 6,
			title: "漫画",
			url: "https://manga.bilibili.com/?from=bill_top_mnav",
		},
		{ id: 7, title: "赛事", url: "https://www.bilibili.com/match/home/" },
		{
			id: 8,
			title: "下载客户端",
...
const RightEntry: React.FC<EntryProps> = ({ entry }) => {
	if (entry.title === "登录") {
		return (
...
export default function HeaderBar() {
	return (
		<div className="bili-header__bar">
			<HeaderBarLeftEntries />
			<CenterSearch />
			<HeaderBarRightEntries />
		</div>
	);
}
